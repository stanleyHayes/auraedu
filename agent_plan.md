...
|---|---|---|---|---|
| AURA-46.14 | EP-46 Organizational Duties + Marketing Depth | **Done** | Kimi Code CLI | Closed every school-admin duty gap where the backend existed but no UI did, guided by the spec audit and xtiitch's production launch patterns. School admins can now manage notification templates (unblocking the journeys builder dead-end), grading scales with validated bands, CSV bulk student/guardian import with client-side preview and per-row results (making the user-guide promise true), per-user permission matrices against the identity catalogue with grant-validation errors surfaced, fee structures plus multi-student invoice issuing, report-card templates with a draft→generate→publish→archive workflow, website CMS pages with a type-aware section editor and reorder, and a weekly timetable grid with conflict surfacing. Teachers can now author CBT exams end to end (creation, draft→published→active→closed lifecycle, question bank with MC/true-false/short-answer, attach/reorder), making the previously dead cbt_exams feature usable. Marketing gains a tabbed product tour with designed screenshot placeholders pending real captures, a fee-lifecycle 'term in motion' mock reused across pages, FAQ, Terms and payment-policy pages marked pending the legal-review sign-off gate, a for-parents viral-adoption page, Organization/WebSite/SoftwareApplication JSON-LD, concrete proof stats, the admissions growth loop, and WhatsApp/department contact channels with real idempotent server submission through the onboarding intake. Web 141 tests, marketing 17 tests, lint, typecheck, Prettier and both production builds pass. |
| AURA-46.13 | EP-46 Platform Oversight Consoles | **Done** | Kimi Code CLI | Closed the platform/school oversight gaps found in an RBAC audit. The super-admin console gains per-school drill-down (tabbed tenant detail with students, staff, finance, attendance, audit and delivery subpages pinned via `X-Tenant-Code`), an onboarding approve/reject pipeline wired to the existing contract endpoints, a cross-tenant audit explorer with an all-tenants option, full billing plan CRUD and subscription management against the already-contracted mutations, and a per-school dashboard overview table with honest capped counts. The school-admin portal gains a full audit-log viewer, identity user management (invite, role reassignment, deactivation with grant-validation errors surfaced), read-only CBT exam/submission oversight, pending-inclusive AI oversight pages for recommendations/predictions/career guidance, and a notification delivery-log surface with unmistakable outcome states. Contracts-first audit filtering adds `event_type`/`actor_id`/`source_service`/`from`/`to` parameters with validated 422s through `make contracts` codegen, the audit service's tenant-scoped and cross-tenant repositories, and keyset pagination intact; the gateway audit route is now TenantOptional so platform super admins reach the existing cross-tenant `ListAll` path while tenantless non-platform actors fail closed by regression test. School admins now receive `ai.agent.configure`/`ai.action.approve` at bootstrap plus an additive 0019 backfill migration, unbreaking the existing `/admin/automation` control room. Regenerating shared types flushed out committed assessment-contract drift (`title`/`class_ids`/`due_date`); the five stale portal consumers were updated to the real service payload. Web 85 tests, lint, typecheck, Prettier, the 77-route production build, golangci-lint and full Go unit/Postgres suites for audit/identity/gateway, contracts lint, migration inventory, RLS and security scans all pass. |
...
| AURA-4.18 | EP-04 Platform Administrator Account Management | **Done** | Codex + Claude Code | Made the tenantless platform identity from AURA-4.17 actually operable instead of only representable. The Gateway's login, refresh, MFA and recovery routes plus the identity administration routes (`/users`, `/roles`, `/permissions`) are now tenant-optional, but only when no tenant is supplied: an explicit `X-Tenant-Code`/`X-Tenant-ID` still resolves and still enforces the actor/tenant match, so school sign-in and recovery remain tenant-bound. Identity's recovery is scope-symmetric — a tenantless request may only issue and consume a tenantless token and a school request only its own — and PostgreSQL's reset predicates use `IS NOT DISTINCT FROM` so a NULL platform tenant matches itself rather than nothing. The directory still fails closed: an actor without a resolved tenant that is not a platform admin is denied rather than widened across tenants. Added `/superadmin/users` (identity directory with scope, role, permission and status controls) and `/superadmin/settings` (profile, recovery, MFA posture, and a persisted Aura/neumorphic/clay/liquid-glass surface preference applied before paint and reduced for `prefers-reduced-transparency`). Shipped an accessible `PasswordInput` across every portal password form, fixed the sidebar to resolve one most-specific active destination, and seeded linked demo teacher/parent/student identities so every role portal is exercisable locally. Six Identity tenant-isolation tests, two Gateway platform/school login tests, 147 web tests, `go vet`, lint, typecheck and the production Next.js build are green. |
...
| AURA-9.8 | EP-09 Vercel Frontend Topology | **In progress** | Codex | Aligned hosting with the user directive: Render is backend-only and exposes only API Gateway publicly; its duplicate paid Web and Marketing services were removed. Both Next.js apps have separate app-root `vercel.json` projects that install from the pnpm workspace root, build only their owning app in Frankfurt, and run a fail-closed production-origin validator before compilation. The validator rejects missing, loopback, plaintext, credential-bearing, query/fragment-bearing and path-bearing public origins; three regressions pass. Added `tools/vercelprobe`, a bounded and secret-minimizing AURA-9.8 evidence producer that requires two independently Git-linked Next.js projects at the exact expected roots, production-scoped environment keys without decrypting values, one `READY` production deployment of the exact release Git SHA per project, recognizable public app content, the configured security-header contract, and exact API Gateway CORS preflights from both distinct frontend origins. Eight probe regressions cover success, secret exclusion, incomplete env, stale SHA, partial CORS, query pinning, placeholder/one-origin rejection, Vercel response variants and immutable output; Go test/vet/build/lint, scenario validation, release validation, security/freshness scans, actionlint and diff checks pass. CI now tests/builds the probe, the local config generator exposes every required `AURA_VERCEL_*` placeholder, and deployment/release runbooks describe evidence capture. Web lint/typecheck/43 tests and a 64-route production build plus Marketing lint/typecheck/8 tests and a 24-route production build passed earlier in this slice. The `auraedugh` Portal and `auraedugh-marketing` projects now exist in the authenticated Vercel team, are independently linked to `stanleyHayes/auraedu` on `main`, use Next.js with exact `apps/web` and `apps/marketing` roots, include workspace source outside those roots, and have affected-project deployment detection enabled. Both Production targets contain `ENVIRONMENT` and the Portal `NEXT_PUBLIC_APP_URL` without exposing their values, and Render's committed Gateway CORS names both generated Vercel origins exactly. The push of `4150015` triggered both Production builds: each cloned the exact commit, installed all 14 workspace projects and detected Next.js 16.2.10 before the production validator rejected only the missing `NEXT_PUBLIC_API_URL`. The root engine range admits Vercel's managed Node 24 build image while local/CI/Docker remain pinned to Node 26.5; a production-mode Node 24.12 compatibility run completes TypeScript and all 69 Portal/24 Marketing routes. Render CLI is now authenticated and the corrected 90-action Blueprint passes Render's live schema, but creating its approximately USD 886.50/month baseline footprint requires explicit spend approval and provider secrets; the public Gateway origin, Vercel Gateway variables, matching `READY` deployments and immutable provider evidence remain. |
| AURA-53.13 | EP-53 Render Network Isolation | **Done** | Codex | Closed a production-default exposure in the Blueprint: Render permits public-IP PostgreSQL connections when `ipAllowList` is omitted, so all 25 service databases now explicitly use an empty allowlist. A fail-closed security gate proves every database and Key Value store denies public ingress, all 28 domain/NATS deployments remain private services, every database binding uses Render's private `connectionString`, workers retain worker topology, and only API Gateway is a public Render web service; Web and Marketing are Vercel-owned. The same invariant is part of the disaster-recovery topology check. Render's live validator accepts all 90 Blueprint actions, and all local Render configuration, workflow-routing and static-security gates pass. |
...
| AURA-9.3 | EP-09 Production Deployment Hardening | **In progress** | Codex | Made deployment safety executable across the full Render topology. NATS no longer inherits Render's deploy-on-commit default: its persistent-disk deployment now waits for linked CI and rebuilds only for its Dockerfile or Blueprint. Render's live validator exposed that the former local gate accepted provider-invalid private health checks, Docker `startCommand` fields and untyped `fromService` references. The Blueprint now omits forbidden private `healthCheckPath` values, uses `dockerCommand` for every Docker worker/cleanup process and declares the exact target type on every service reference; the scaffold emits the same valid shape. The strengthened gate proves all 56 deployable services use `checksPass`, every Docker deployment has a scoped build covering its Dockerfile, every public service has an absolute health check, private services do not declare unsupported health checks, commands/references match the provider schema, plans are explicit and paid, regions are Frankfurt and environment keys are unique. Render now reports `valid: true` for all 90 planned actions. Resource-class sizing, autoscaling and provider-observed zero-downtime evidence remain incomplete because creating the approximately USD 886.50/month baseline fleet requires explicit spend approval. |
| AURA-9.4 | EP-09 Managed Backups + Disaster Recovery | **In progress** | Codex | Replaced the placeholder DR chapter with explicit Tier 0–2 RPO/RTO targets, accountable incident roles, provider/logical/NATS recovery architecture, cutover/rollback rules and a production-oriented runbook. CI proves all 25 Render databases remain paid, pinned PostgreSQL 18 services and NATS retains its minimum 10 GB `/data` disk; mutable recovery images fail the security scan. The PostgreSQL and source-destruction JetStream restore drills pass with deterministic data fingerprints. CI-gated recovery crons now cover both authoritative stores: daily `nats-backup` snapshots every file stream and durable consumer through the official pinned CLI, while hourly `postgres-backup` uses private connections to create and catalogue-validate ownership-neutral custom-format exports for all 25 service databases. Both hash artifacts, sign S3-compatible writes with SigV4, require AES-256, non-overwrite and 35-day `COMPLIANCE` Object Lock, and use independent authenticated success/failure monitors; missing storage, monitor or database coverage fails startup or topology CI. The PostgreSQL image is pinned PostgreSQL 18.4 and UID 70; its committed live smoke proves least-privilege database access, real `pg_dump`, `pg_restore --list`, a 1,713-byte TLS upload and accepted heartbeat. NATS unit/race/vet, hardened URL/path validation, nonroot distroless image build and the real account-backup smoke also pass. Full DR/deploy/network/runtime/security gates, Render live-schema validation and both isolated restore drills are green. Provider PITR configuration, successful deployed cron runs, recovery-bucket versioning/Object Lock inspection, received heartbeat/page evidence and a provider-hosted restore/cutover drill remain required before Done. |
...
| AURA-9.7 | EP-09 AI Database Readiness | **Done** | Codex | Replaced shallow traffic health for all three Python AI services with bounded, redacted PostgreSQL readiness. `/health` remains liveness; `/ready` executes `SELECT 1` within two seconds and returns `503 not_ready` without connection details on failure. Compose health checks, Gateway dependency probing and the protected platform dependency dashboard use `/ready`; the local Gateway waits for all three AI databases to become ready, and runtime CI prevents reverting those executable probes to liveness-only routing. Render's live schema does not permit `healthCheckPath` on private services, so the Blueprint intentionally omits that unsupported field while each deployed service still exposes `/ready` for authenticated operational probing. Corrected both Render and Compose PostgreSQL DSN dialect/SSL normalization discovered by the live readiness path. All three rebuilt non-root images reached `healthy` simultaneously against isolated real PostgreSQL and JetStream. Ready/unavailable regressions, Gateway dependency tests/vet, all 78 AI tests, Ruff/mypy, runtime gates, live Blueprint validation and diff checks pass. |
...
| AURA-9.1 | EP-09 Render Deployment | **In progress** | Codex | Corrected scheme-less Render `hostport` handling, invalid env-group secret declarations, production provider selection, Identity worker wiring and server-to-Tenant activation discovery. Gateway `/ready` now runs the actual Redis dependency check instead of a disconnected empty health state, with a regression test. A deployment-wide private-auth audit found Payment, CBT and all three Python AI services would start without the shared internal token, causing their authoritative Fees/Student ownership checks to fail closed in production; all five now receive `auraedu-secrets`. Recommendation's publisher/subscriber also normalize Render's scheme-less NATS hostname before connecting. Frontend production configuration fails closed on unsafe public origins, and Web/Marketing Dockerfiles retain hermetic local/container builds while production deployment is now Vercel-owned. Replaced the removed Node 26 Corepack bootstrap with pinned pnpm and made both standalone runtime images ownership-safe and non-root. The reusable Render gate proves 72 hard requirements across 48 backend deployments and all 25 live feature-gated process bindings; provider-schema rules now reject invalid private health checks, Docker commands and untyped service references. The authenticated Render CLI confirms there are no existing AuraEDU resources and the corrected Blueprint is `valid: true`, comprising 90 planned actions across 25 databases, 7 environment groups, 1 Key Value instance and 57 services/jobs. Config lint/typecheck/security tests, live Blueprint validation, Compose validation, 69-route Web and 24-route Marketing Node 24 production builds, real full-topology Docker readiness, the AuthN/Z matrix, Payment private-client tests, complete CBT suite, and Recommendation tests plus Ruff pass. Deployment remains gated on explicit approval for the approximately USD 886.50/month baseline footprint and entry of the required provider secrets. |
...
| AURA-4.18 | Platform administrator account management | tenant-optional identity routes that still bind an explicitly supplied tenant; scope-symmetric tenantless recovery; directory denied without tenant or platform scope; platform users/settings consoles; accessible password reveal | 5 | AURA-4.17 |
