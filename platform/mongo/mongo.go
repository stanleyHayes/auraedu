...
func (c *Collection) Scope(ctx context.Context) (*Scope, error) {
	...
	return &Scope{coll: c.coll, tenantID: tenantID, platformAdmin: platformAdmin}, nil
}
...
func (c *Collection) ScopeTo(tenantID string) (*Scope, error) {
	...
	return &Scope{coll: c.coll, tenantID: tenantID}, nil
}
...
func (s *Scope) filter(query bson.M) bson.M {
	...
	// Set last so a caller cannot widen its own scope by passing tenant_id.
	merged[TenantField] = s.tenantID
	return merged
}
...
func guardUpdate(update bson.M) error {
	for _, op := range []string{"$set", "$setOnInsert", "$unset", "$rename"} {
		fields, ok := update[op].(bson.M)
		if !ok {
			continue
		}
		if _, touching := fields[TenantField]; touching {
			return fmt.Errorf("mongo: update may not modify %s", TenantField)
		}
	}
	if _, replacing := update[TenantField]; replacing {
		return fmt.Errorf("mongo: update may not modify %s", TenantField)
	}
	return nil
}
