...
func newFile(tenantID string) *domain.FileUpload {
	...
	return &domain.FileUpload{
		...
		// The storage path is uniquely indexed per tenant, so every fixture needs
		// its own or the second insert collides rather than testing anything.
		StoragePath:      "/uploads/" + uuid.NewString() + ".pdf",
