...
CREATE INDEX cbt_outbox_pending ON cbt_outbox(next_attempt_at, created_at) WHERE published_at IS NULL;
ALTER TABLE cbt_outbox ENABLE ROW LEVEL SECURITY;
ALTER TABLE cbt_outbox FORCE ROW LEVEL SECURITY;
CREATE POLICY cbt_outbox_tenant_isolation ON cbt_outbox FOR ALL
...
-- +goose StatementEnd
-- +goose Down
DROP TABLE IF EXISTS cbt_outbox;
