...
import (
	...
	adapter "github.com/auraedu/report-service/internal/adapters/mongo"
...
func openMongo(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	m, err := pmongo.Open(ctx, pmongo.Config{URI: uri, Database: config.Getenv("MONGODB_DATABASE", "auraedu_report"), MaxPoolSize: poolSize})
	if err != nil {
		return nil, err
	}
	r.close = func() {
