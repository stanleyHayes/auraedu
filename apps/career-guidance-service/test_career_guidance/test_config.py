...
import pytest
from career_guidance_service.config import Settings, postgres_dsn, validate_production_runtime
...
def test_production_runtime_rejects_local_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="AI_GUIDANCE_DATABASE_URL"):
