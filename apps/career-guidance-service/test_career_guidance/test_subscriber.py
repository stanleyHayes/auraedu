"""Tests for the Career Guidance feature-store subscriber topology."""

import json
from types import SimpleNamespace
from typing import Any

from career_guidance_service.events import subscriber as subscriber_module
from career_guidance_service.events.subscriber import (
    DURABLE_PREFIX,
    MAX_DELIVERIES,
    MAX_EVENT_BYTES,
...
class RecordingJetStream:
    ...
    async def subscribe(self, subject: str, **options: Any) -> object:  # noqa: ARG002
        self.durables.append(options["durable"])
...
async def test_guidance_uses_service_owned_durable_consumers() -> None:
    ...
    assert DURABLE_PREFIX == "ai-guidance"
    assert len(jetstream.durables) == len(SUBSCRIBED_EVENTS)
    assert all(name.startswith("ai-guidance-") for name in jetstream.durables)
    assert not any(name.startswith("ai-pred-") for name in jetstream.durables)
