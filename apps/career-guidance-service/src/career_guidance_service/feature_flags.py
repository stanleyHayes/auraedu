...
class FeatureGate:
    ...
    def is_enabled(self, tenant_id: str, feature_key: str) -> bool:
        ...
        if features is None:
            # Domain services remain private, but still fail closed behind the
            # gateway when the tenant snapshot is missing or stale.
            return False
...
def _build_gate() -> FeatureGate:
    path = os.getenv("AI_GUIDANCE_FEATURES_REGISTRY", DEFAULT_REGISTRY)
    try:
...
def _remote_enabled(tenant_id: str, feature_key: str) -> bool | None:
    base_url = os.getenv("AI_GUIDANCE_TENANT_SERVICE_URL", "").rstrip("/")
    if not base_url:
        return None
    if "://" not in base_url:
