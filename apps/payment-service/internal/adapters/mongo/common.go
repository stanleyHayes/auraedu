...
func remove(ctx context.Context, c *pmongo.Collection, tenant, id string, events ...tenancy.CloudEvent) error {
	scope, err := c.ScopeTo(tenant)
	if err != nil {
		return err
	}
	// SQL's payment/transaction FK is RESTRICT. Both records are embedded here,
	// so the delete predicate also fences a concurrent reconciliation/ledger insert.
	result,
		err := scope.UpdateWithEvents(ctx,
		live(bson.M{"_id": id,
			"transactions.0": bson.M{"$exists": false}}),
	...
	if _, err := get[domain.Payment](ctx, c, tenant, bson.M{"_id": id}); err != nil {
		return err
	}
	return domain.ErrConflict
}
...
func indexes(ctx context.Context, s *pmongo.Store, specs map[string][]mongo.IndexModel) error {
	for name, models := range specs {
		if _, err := s.Database().Collection(name).Indexes().CreateMany(ctx, models); err != nil {
			return fmt.Errorf("payment: index %s: %w", name, err)
		}
	}
	return nil
}
