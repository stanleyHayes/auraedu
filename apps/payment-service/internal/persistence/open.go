...
import (
	...
	adapter "github.com/auraedu/payment-service/internal/adapters/mongo"
...
type Repositories struct {
	...
	Webhook     ports.WebhookEventRepository
	Driver      store.Driver
	close       func()
...
func openMongo(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	m, err := pmongo.Open(ctx, pmongo.Config{URI: uri, Database: config.Getenv("MONGODB_DATABASE", "auraedu_payment"), MaxPoolSize: poolSize})
	if err != nil {
		return nil, err
	}
	r.close = func() {
	...
	r.Payment = adapter.NewPaymentRepository(m)
	r.Transaction = adapter.NewTransactionRepository(m)
	r.Webhook = adapter.NewWebhookEventRepository(m)
	return r, nil
}
func openPostgres(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	r.close = p.Close
	r.ping = p.Ping
	r.Payment = postgres.NewPaymentRepository(p)
	r.Transaction = postgres.NewTransactionRepository(p)
	r.Webhook = postgres.NewWebhookEventRepository(p)
	return r, nil
}
