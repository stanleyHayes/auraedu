// Package workercmd dispatches knowledge lifecycle events.
package workercmd
...
import (
	...
	"github.com/auraedu/knowledge-service/internal/adapters/events"
...
const serviceName = "knowledge-service-worker"
...
func Run(version string) error {
	...
	defer cancel()
	shutdownTelemetry, err := observ.InitTracing(serviceName, version)
	if err != nil {
		return fmt.Errorf("knowledge worker telemetry: %w", err)
	}
	defer func() {
		shutdownCtx, shutdownCancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer shutdownCancel()
		if err := shutdownTelemetry(shutdownCtx); err != nil {
			log.Error("flush knowledge worker telemetry", "err", err)
		}
	}()
	...
	if err != nil {
		return fmt.Errorf("knowledge worker connect NATS: %w", err)
	}
	defer nc.Close()
	js, err := nc.JetStream()
	if err != nil {
		return fmt.Errorf("knowledge worker JetStream: %w", err)
	}
	if _, err := eventbus.EnsureStream(js, "AURA"); err != nil {
		return fmt.Errorf("knowledge worker event stream: %w", err)
	}
	...
	defer ticker.Stop()
	log.Info("knowledge outbox worker started", "version", version)
	for {
		started := time.Now()
		err := dispatch(ctx, repo, pub, log, metrics)
		metrics.Observe(ctx, "outbox-batch", started, err)
		if err != nil && ctx.Err() == nil {
			log.Error("knowledge outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func dispatch(ctx context.Context, repo outboxRepository, pub outboxPublisher, log *slog.Logger, metrics *observ.WorkerMetrics) error {
	...
	for _, item := range items {
		...
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			...
			log.Warn("knowledge event publish deferred", "outbox_id", item.ID, "event_type", item.EventType, "err", err)
			continue
		}
