...
import (
	...
	adapter "github.com/auraedu/staff-service/internal/adapters/mongo"
...
func Open(ctx context.Context) (*Connection, error) {
	...
	if driver.IsMongo() {
		s, err := pmongo.Open(ctx, pmongo.Config{URI: config.Getenv("MONGODB_URI", ""), Database: config.Getenv("MONGODB_DATABASE", "auraedu_staff"), MaxPoolSize: 2})
		if err != nil {
			return nil, err
		}
