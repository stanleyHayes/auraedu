// Package workercmd dispatches durable Staff Service outbox events.
package workercmd
...
const service = "staff-service-worker"

type outboxPublisher interface {
	PublishWithID(context.Context, string, string, string, map[string]any) error
}

// Run dispatches durable staff lifecycle events.
func Run(version string) error {
	...
	if err != nil {
		return fmt.Errorf("staff worker: connect NATS: %w", err)
	}
	defer nc.Close()
	js, err := nc.JetStream()
	if err != nil {
	...
	for {
		...
		if err != nil && ctx.Err() == nil {
			log.Error("staff outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func dispatch(ctx context.Context, repo ports.OutboxRepository, pub outboxPublisher, metrics *observ.WorkerMetrics) error {
	items, err := repo.ClaimPendingStaffEvents(ctx, 25)
	if err != nil {
		return err
	}
	for _, item := range items {
		...
		if err := json.Unmarshal(item.Payload, &payload); err != nil {
			...
			if markErr := repo.MarkStaffEventFailed(ctx, item.ID, "invalid outbox payload"); markErr != nil {
		...
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			metrics.Observe(ctx, "outbox-publish", started, err)
			if markErr := repo.MarkStaffEventFailed(ctx, item.ID, err.Error()); markErr != nil {
				return errors.Join(err, markErr)
			}
			continue
		}
		if err := repo.MarkStaffEventPublished(ctx, item.ID); err != nil {
			return err
		}
