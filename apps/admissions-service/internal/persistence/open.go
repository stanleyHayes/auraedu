...
import (
	...
	adapter "github.com/auraedu/admissions-service/internal/adapters/mongo"
...
type Repository interface {
	ports.Repository
	ports.TransactionalRepository
	ports.CatalogueRepository
	ports.CatalogueTransactionalRepository
	ports.CatalogueEventRepository
...
func Open(ctx context.Context) (*Connection, error) {
	...
	if driver.IsMongo() {
		s, err := pmongo.Open(ctx, pmongo.Config{URI: config.Getenv("MONGODB_URI",
			""), Database: config.Getenv("MONGODB_DATABASE", "admissions-service"),
			MaxPoolSize: 2})
		if err != nil {
			return nil, err
		}
