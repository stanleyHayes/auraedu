// Package workercmd provides the Admissions Service outbox worker command.
package workercmd
...
func Run() error {
	...
	defer cancel()
	shutdownTelemetry, err := observ.InitTracing("admissions-service-worker", config.Getenv("GIT_SHA", "dev"))
	if err != nil {
		return fmt.Errorf("admissions worker telemetry: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return fmt.Errorf("admissions worker nats: %w", err)
	}
	defer nc.Close()
	js, err := nc.JetStream()
	if err != nil {
	...
	defer ticker.Stop()
	metrics := observ.NewWorkerMetrics("admissions-service-worker", "outbox-batch", "outbox-publish")
	for {
		started := time.Now()
		err := dispatch(ctx, repo, pub, log, metrics)
		metrics.Observe(ctx, "outbox-batch", started, err)
		if err != nil && ctx.Err() == nil {
			log.Error("admissions outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
	...
	for _, item := range items {
		...
		if err := json.Unmarshal(item.Payload, &payload); err != nil {
			...
			if markErr := repo.MarkFailed(ctx, item.ID, err.Error()); markErr != nil {
				return fmt.Errorf("mark malformed admissions event failed: %w", markErr)
			}
			continue
		}
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			...
			log.Warn("admissions event deferred", "outbox_id", item.ID, "err", err)
			continue
		}
