...
import (
	...
	adapter "github.com/auraedu/audit-service/internal/adapters/mongo"
...
type Repository interface{ ports.Repository }
...
func Open(ctx context.Context) (*Connection, error) {
	...
	if driver.IsMongo() {
		s, err := pmongo.Open(ctx, pmongo.Config{URI: config.Getenv("MONGODB_URI", ""), Database: config.Getenv("MONGODB_DATABASE", "auraedu_audit"), MaxPoolSize: 2})
		if err != nil {
			return nil, err
		}
