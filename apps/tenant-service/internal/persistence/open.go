...
import (
	...
	adapter "github.com/auraedu/tenant-service/internal/adapters/mongo"
...
func openMongo(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	m, err := pmongo.Open(ctx, pmongo.Config{URI: uri, Database: config.Getenv("MONGODB_DATABASE", "auraedu_tenant"), MaxPoolSize: poolSize})
	if err != nil {
		return nil, err
	}
	r.close = func() {
