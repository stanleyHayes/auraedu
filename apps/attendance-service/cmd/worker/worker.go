...
const service = "attendance-service-worker"

type outboxPublisher interface {
	PublishWithID(context.Context, string, string, string, map[string]any) error
}

// Run starts the attendance lifecycle outbox dispatcher.
func Run() error {
	log := observ.DefaultLogger()
	ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer cancel()

	shutdown, err := observ.InitTracing(service, "")
	if err != nil {
		return err
	}
	defer func() {
	...
	if err != nil {
		return fmt.Errorf("attendance worker: connect NATS: %w", err)
	}
	defer nc.Close()
	...
	for {
		...
		if err != nil && ctx.Err() == nil {
			log.Error("attendance outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func dispatch(ctx context.Context, repo ports.OutboxRepository, pub outboxPublisher, metrics *observ.WorkerMetrics) error {
	items, err := repo.ClaimPendingAttendanceEvents(ctx, 25)
	if err != nil {
		return err
	}
	for _, item := range items {
		...
		if err := json.Unmarshal(item.Payload, &payload); err != nil {
			...
			if markErr := repo.MarkAttendanceEventFailed(ctx, item.ID, "invalid outbox payload"); markErr != nil {
		...
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			metrics.Observe(ctx, "outbox-publish", started, err)
			if markErr := repo.MarkAttendanceEventFailed(ctx, item.ID, err.Error()); markErr != nil {
				return errors.Join(err, markErr)
			}
			continue
		}
		if err := repo.MarkAttendanceEventPublished(ctx, item.ID); err != nil {
			return err
		}
