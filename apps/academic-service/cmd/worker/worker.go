// Package workercmd provides the academic-service outbox worker command.
package workercmd
...
const service = "academic-service-worker"
...
func Run() error {
	...
	if err != nil {
		return fmt.Errorf("academic worker: connect NATS: %w", err)
	}
	defer nc.Close()
	js, err := nc.JetStream()
	if err != nil {
	...
	for {
		...
		if err != nil && ctx.Err() == nil {
			log.Error("academic outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func dispatch(ctx context.Context, repo ports.OutboxRepository, pub outboxPublisher, metrics *observ.WorkerMetrics) error {
	items, err := repo.ClaimPendingAcademicEvents(ctx, 25)
	if err != nil {
		return err
	}
	for _, item := range items {
		...
		if err := json.Unmarshal(item.Payload, &payload); err != nil {
			...
			if markErr := repo.MarkAcademicEventFailed(ctx, item.ID, "invalid outbox payload"); markErr != nil {
		...
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			metrics.Observe(ctx, "outbox-publish", started, err)
			if markErr := repo.MarkAcademicEventFailed(ctx, item.ID, err.Error()); markErr != nil {
				return errors.Join(err, markErr)
			}
			continue
		}
		if err := repo.MarkAcademicEventPublished(ctx, item.ID); err != nil {
			return err
		}
