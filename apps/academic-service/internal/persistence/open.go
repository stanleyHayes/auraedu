...
import (
	...
	adapter "github.com/auraedu/academic-service/internal/adapters/mongo"
...
type Repositories struct {
	...
	Term      ports.TermRepository
	Class     ports.ClassRepository
	Subject   ports.SubjectRepository
...
func openMongo(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	m, err := pmongo.Open(ctx, pmongo.Config{URI: uri, Database: config.Getenv("MONGODB_DATABASE", "auraedu_academic"), MaxPoolSize: poolSize})
	if err != nil {
		return nil, err
	}
	r.close = func() {
	...
	r.Year = adapter.NewRepository(m)
	r.Term = adapter.NewTermRepository(m)
	r.Class = adapter.NewClassRepository(m)
	r.Subject = adapter.NewSubjectRepository(m)
	r.Timetable = adapter.NewTimetableRepository(m)
	r.Grading = adapter.NewGradingScaleRepository(m)
	return r, nil
}
func openPostgres(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	r.close = p.Close
	r.ping = p.Ping
	r.Year = postgres.NewRepository(p)
	r.Term = postgres.NewTermRepository(p)
	r.Class = postgres.NewClassRepository(p)
	r.Subject = postgres.NewSubjectRepository(p)
	r.Timetable = postgres.NewTimetableRepository(p)
	r.Grading = postgres.NewGradingScaleRepository(p)
	return r, nil
}
