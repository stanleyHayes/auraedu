"""Private Student Service client for prediction learner-scope enforcement."""
...
class LearnerScopeUnavailableError(RuntimeError):
    """Student ownership or teacher assignment could not be resolved safely."""
...
async def resolve_learner_ids(tenant_id: str, user_id: str, role: str) -> set[str]:
    ...
    if not service_token:
        raise LearnerScopeUnavailableError("learner scope credentials are not configured")  # noqa: TRY003
    ...
    try:
        ...
        raise LearnerScopeUnavailableError("learner scope service is unavailable") from exc  # noqa: TRY003
    if status_code != HTTPStatus.OK:
        raise LearnerScopeUnavailableError("learner scope could not be resolved")  # noqa: TRY003
    try:
        body = json.loads(raw_body)
        return {str(value) for value in body.get("student_ids", []) if value}
    except (AttributeError, TypeError, ValueError, json.JSONDecodeError) as exc:
        raise LearnerScopeUnavailableError("learner scope returned an invalid response") from exc  # noqa: TRY003
