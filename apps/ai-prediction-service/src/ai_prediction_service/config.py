...
def validate_production_runtime(current: Settings = settings) -> None:
    ...
    if os.getenv("ENVIRONMENT", "development").strip().lower() != "production":
        return
    values = {
        "AI_PRED_DATABASE_URL": current.database_url,
        "AI_PRED_NATS_HOST": current.nats_host,
        "AI_PRED_STUDENT_SERVICE_URL": current.student_service_url,
        "AI_PRED_TENANT_SERVICE_URL": os.getenv("AI_PRED_TENANT_SERVICE_URL", ""),
    ...
    if current.debug:
        raise ProductionConfigError("AI_PRED_DEBUG", "must be disabled in production")
