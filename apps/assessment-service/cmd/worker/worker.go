// Package workercmd runs the durable assessment outbox worker.
package workercmd
...
import (
	...
	svcevents "github.com/auraedu/assessment-service/internal/adapters/events"
	"github.com/auraedu/assessment-service/internal/persistence"
...
const service = "assessment-service-worker"
...
func Run(version string) error {
	...
	url, err := config.MustGetenv("NATS_URL")
	if err != nil {
		return err
	}
	nc, err := nats.Connect(url, nats.Timeout(5*time.Second))
	if err != nil {
		return fmt.Errorf("assessment worker: connect NATS: %w", err)
	}
	defer nc.Close()
	js, err := nc.JetStream()
	if err != nil {
	...
	for {
		...
		if err != nil && ctx.Err() == nil {
			log.Error("assessment outbox dispatch failed", "err", err)
		}
		select {
		case <-ctx.Done():
			return nil
...
func dispatch(ctx context.Context, repo ports.OutboxRepository, pub outboxPublisher, metrics *observ.WorkerMetrics) error {
	items, err := repo.ClaimPendingAssessmentEvents(ctx, 25)
	if err != nil {
		return err
	}
	for _, item := range items {
		...
		if err := json.Unmarshal(item.Payload, &payload); err != nil {
			...
			if markErr := repo.MarkAssessmentEventFailed(ctx, item.ID, "invalid outbox payload"); markErr != nil {
		...
		if err := pub.PublishWithID(ctx, item.ID, item.EventType, item.TenantID, payload); err != nil {
			metrics.Observe(ctx, "outbox-publish", started, err)
			if markErr := repo.MarkAssessmentEventFailed(ctx, item.ID, err.Error()); markErr != nil {
				return errors.Join(err, markErr)
			}
			continue
		}
		if err := repo.MarkAssessmentEventPublished(ctx, item.ID); err != nil {
			return err
		}
