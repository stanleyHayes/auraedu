...
import (
	...
	adapter "github.com/auraedu/billing-service/internal/adapters/mongo"
...
type Repositories struct {
	...
	Invoice ports.SaaSInvoiceRepository
	Driver  store.Driver
	close   func()
...
func openMongo(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	m, err := pmongo.Open(ctx, pmongo.Config{URI: uri, Database: config.Getenv("MONGODB_DATABASE", "auraedu_billing"), MaxPoolSize: poolSize})
	if err != nil {
		return nil, err
	}
	r.close = func() {
	...
	r.Plan = adapter.NewPlanRepository(m)
	r.Subscription = adapter.NewSubscriptionRepository(m)
	r.Invoice = adapter.NewSaaSInvoiceRepository(m)
	return r, nil
}
func openPostgres(ctx context.Context, r *Repositories) (*Repositories, error) {
	...
	r.close = p.Close
	r.ping = p.Ping
	r.Plan = postgres.NewPlanRepository(p)
	r.Subscription = postgres.NewSubscriptionRepository(p)
	r.Invoice = postgres.NewSaaSInvoiceRepository(p)
	return r, nil
}
