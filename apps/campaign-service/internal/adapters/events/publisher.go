// Package events publishes Campaign lifecycle events.
package events
...
func (p *Publisher) PublishWithID(ctx context.Context, eventID, eventType, tenantID string, data map[string]any) error {
	...
	event, e := tenancy.NewCloudEvent(eventType, "campaign-service", eventID, tenantID, data)
	if e != nil {
		return fmt.Errorf("campaign event: %w", e)
	}
